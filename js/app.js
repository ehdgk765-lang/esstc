...
const App = {
  ...
  _renderTimeCourtForm(container) {
    ...
    container.querySelector('#schedule-form').onsubmit = (e) => {
      ...
      if (isManualMode) {
        ...
        // 사전 검증: 배분 가능한지 확인
        const testResult = Schedule.distributeTypesToSlots(typeDistribution, slotsForValidation.length, courts, selectedMales.length, selectedFemales.length);
        if (!testResult) {
      ...
      const timeSlots = Schedule.generate(selectedMales, selectedFemales, courts, startTime, endTime, allowMixed, isSingles, allowXD, null, typeDistribution, 10, 25);
  ...
  updateSchedulePreview(container) {
    ...
    if (isSingles) {
      ...
      if (allowXD && maleCount >= 2 && femaleCount >= 2) possibleTypeCodes.push('XD');
      ...
      const updateTotal = () => {
        ...
        if (sum === totalGamesMax) {
          ...
          if (maleCount > 0 && fSlots > 0 && femaleCount > 0) {
            ...
            balanceText = `예상 게임수: 남 ${Math.floor(mAvg)}~${Math.ceil(mAvg)}회, 여 ${Math.floor(fAvg)}~${Math.ceil(fAvg)}회`;
